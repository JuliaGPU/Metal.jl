...
@testset "rand($T), seed $seed" for T in (Int32, UInt32, Int64, UInt64,
                                          Float16, Float32),
    ...
    @testset "across calls" begin
        ...
        @metal threads=n kernel(a, b, seed)
    ...
    @testset "across threads" for active_dim in 1:6
        ...
        @metal threads=(tx, ty, tz) groups=(bx, by, bz) kernel(a, seed)
...
@testset "basic randn($T), seed $seed" for T in (Float16, Float32, ComplexF16, ComplexF32),
                                           seed in (nothing, #=missing,=# 1234)
    function kernel(A::AbstractArray{T}, seed) where {T}
        apply_seed(seed)
        tid = thread_position_in_grid().x
...
@testset "basic randexp($T), seed $seed" for T in (Float16, Float32),
                                           seed in (nothing, #=missing,=# 1234)
    function kernel(A::AbstractArray{T}, seed) where {T}
        apply_seed(seed)
        tid = thread_position_in_grid().x
