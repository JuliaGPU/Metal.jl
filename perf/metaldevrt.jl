...
function kernel_add_mat_vec(m, x1, x2, y)
    ...
    offset = (threadgroup_position_in_grid().x-1) * m
    @inbounds xtmp = x2[threadgroup_position_in_grid().x]
    for i = thread_position_in_threadgroup().x : threadgroups_per_grid().x : m
