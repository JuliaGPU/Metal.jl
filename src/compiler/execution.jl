...
function _metal_launch(@nospecialize(kernel::HostKernel), @nospecialize(args::Tuple),
    ...
    (ts.width * ts.height * ts.depth) > pipeline.maxTotalThreadsPerThreadgroup &&
        throw(ArgumentError("Number of threads in group ($(ts.width * ts.height * ts.depth)) should not exceed $(pipeline.maxTotalThreadsPerThreadgroup)"))
