...
function Base.show(io::IO, I::StaticCartesianIndices)
    print(io, "Static")
    return show(io, CartesianIndices(I))
end
...
@inline function _copyto!(dest::AbstractArray, bc::Broadcasted)
    ...
    if ndims(dest) == 1 || (isa(IndexStyle(dest), IndexLinear) && isa(IndexStyle(bc), IndexLinear))
    ...
    elseif ndims(dest) == 3
        ...
        w = min(size(dest, 1), kernel.pipeline.threadExecutionWidth)
        h = min(
            size(dest, 2), kernel.pipeline.threadExecutionWidth,
