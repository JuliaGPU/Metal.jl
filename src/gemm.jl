...
# scalar multiply/AXPY is just an elementwise op on the tuple — no intrinsic needed.
...
@inline _cvt(::Type{R}, m) where {R} = ntuple(i -> VecElement{R}(R(m[i].value)), Val(64))
@inline _scale(m, a) = ntuple(i -> VecElement{Float32}(a * m[i].value), Val(64))
@inline _axpby(m, c, a, b) = ntuple(i -> VecElement{Float32}(a * m[i].value + b * c[i].value), Val(64))
...
@inline _load_afrag(As, sm0, ks, ::Val{TM}) where {TM} =
    ntuple(ti -> simdgroup_load(As, (sm0 + (ti - 1) * 8 + 1, ks + 1)), Val(TM))
@inline _load_bfrag(Bs, sn0, ks, ::Val{TN}) where {TN} =
    ntuple(tj -> simdgroup_load(Bs, (ks + 1, sn0 + (tj - 1) * 8 + 1)), Val(TN))
@inline _mma(acc, afrag, bfrag, ::Val{TM}, ::Val{TN}) where {TM, TN} =
...
@inline function _store_frag!(C::MtlDeviceArray{R}, accidx, gr, gc, alpha, beta,
                              ::Val{SIMPLE}) where {R, SIMPLE}
    if SIMPLE
        simdgroup_store(_cvt(R, accidx), C, (gr + 1, gc + 1))
    elseif iszero(beta)
        simdgroup_store(_cvt(R, _scale(accidx, alpha)), C, (gr + 1, gc + 1))
    else
        cb = simdgroup_load(C, (gr + 1, gc + 1))
        simdgroup_store(_cvt(R, _axpby(accidx, _cvt(Float32, cb), alpha, beta)), C, (gr + 1, gc + 1))
    end
    return
end
...
@inline function _epilogue!(C::MtlDeviceArray{R}, scratch, acc, bm0, sm0, bn0, sn0, M, N,
    ...
    ntuple(Val(TM * TN)) do idx
        ...
        if !EDGE || (gr + 8 <= M && gc + 8 <= N)
            _store_frag!(C, acc[idx], gr, gc, alpha, beta, Val(SIMPLE))
        else
...
function _gemm_simd_kernel!(C, A, B, alpha::Float32, beta::Float32,
    ...
    while kt0 < K
        ...
        ks = 0
        while ks < BK
            afrag = _load_afrag(As, sm0, ks, Val(TM))
            bfrag = _load_bfrag(Bs, sn0, ks, Val(TN))
            acc = _mma(acc, afrag, bfrag, Val(TM), Val(TN))
            ks += 8
        end
        threadgroup_barrier(MemoryFlagThreadGroup)
        kt0 += BK
    ...
    _epilogue!(C, scratch, acc, bm0, sm0, bn0, sn0, M, N, alpha, beta, lane, s0 * 8,
