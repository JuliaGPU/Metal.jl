...
function Base.unsafe_copyto!(
        dev::MTLDevice, dst::MtlPtr{T}, src::Ptr{T}, N::Integer;
        queue::MTLCommandQueue = global_queue(dev), async::Bool = false
    ) where {T}
    ...
    if storage_type == MTL.MTLStorageModePrivate
        ...
        unsafe_copyto!(
            dev, MtlPtr{T}(dst.buffer, dst.offset), MtlPtr{T}(tmp_buf, 0), N;
...
# GPU -> CPU
function Base.unsafe_copyto!(
        dev::MTLDevice, dst::Ptr{T}, src::MtlPtr{T}, N::Integer;
        queue::MTLCommandQueue = global_queue(dev), async::Bool = false
    ) where {T}
    ...
    if storage_type == MTL.MTLStorageModePrivate
        ...
        unsafe_copyto!(
            dev, MtlPtr{T}(tmp_buf, 0), MtlPtr{T}(src.buffer, src.offset), N;
            queue, async = (nocopy && async)
        )
...
# GPU -> GPU
@autoreleasepool function Base.unsafe_copyto!(
        dev::MTLDevice, dst::MtlPtr{T},
...
@autoreleasepool function unsafe_fill!(
        dev::MTLDevice, dst::MtlPtr{T},
