...
function MTLLibrary(
        dev::MTLDevice, src::String,
        opts::MTLCompileOptions = MTLCompileOptions()
    )
    err = Ref{id{NSError}}(nil)
    handle = @objc [
        dev::id{MTLDevice} newLibraryWithSource:src::id{NSString}
...
function MTLLibraryFromFile(dev::MTLDevice, path::String)
    ...
    handle = let
        url = NSFileURL(path)
        @objc [
            dev::id{MTLDevice} newLibraryWithURL:url::id{NSURL}
            error:err::Ptr{id{NSError}}
        ]::id{MTLLibrary}
    end
    err[] == nil || throw(NSError(err[]))
...
function MTLLibraryFromData(dev::MTLDevice, input_data)
    ...
    GC.@preserve input_data begin
        ...
        handle = @objc [
            dev::id{MTLDevice} newLibraryWithData:data::dispatch_data_t
            error:err::Ptr{id{NSError}}
        ]::id{MTLLibrary}
    end
    err[] == nil || throw(NSError(err[]))
