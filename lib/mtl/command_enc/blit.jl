...
function MTLBlitCommandEncoder(cmdbuf::MTLCommandBuffer)
    ...
    return MTLBlitCommandEncoder(handle)
end
...
function append_copy!(
        enc::MTLBlitCommandEncoder, dst::MTLBuffer, doff,
        src::MTLBuffer, soff, len
    )
    return @objc [
        enc::id{MTLBlitCommandEncoder} copyFromBuffer:src::id{MTLBuffer}
...
for T in (UInt8, Int8)
    @eval begin
        function append_fillbuffer!(
                enc::MTLBlitCommandEncoder, src::MTLBuffer,
                val::$T, bytesize, offset = 0
            )
            range = NSRange(offset, bytesize)
            return @objc [
                enc::id{MTLBlitCommandEncoder} fillBuffer:src::id{MTLBuffer}
                range:range::NSRange
                value:val::$T
            ]::Nothing
        end
...
function append_sync!(enc::MTLBlitCommandEncoder, src::MTLBuffer)
    return @objc [enc::id{MTLBlitCommandEncoder} synchronizeResource:src::id{MTLBuffer}]::Nothing
end
