...
function MPSMatrixDecompositionCholesky(dev, lower, order)
    ...
    finalizer(release, obj)
    @objc [
        obj::id{MPSMatrixDecompositionCholesky} initWithDevice:dev::id{MTLDevice}
        lower:lower::Bool
        order:order::NSUInteger
    ]::id{MPSMatrixDecompositionCholesky}
    return obj
end

function encode!(cmdbuf::MTLCommandBuffer, kernel::MPSMatrixDecompositionCholesky, sourceMatrix, resultMatrix, status)
    return @objc [
        kernel::id{MPSMatrixDecompositionCholesky} encodeToCommandBuffer:cmdbuf::id{MTLCommandBuffer}
        sourceMatrix:sourceMatrix::id{MPSMatrix}
        resultMatrix:resultMatrix::id{MPSMatrix}
        status:status::id{MTLBuffer}
    ]::Nothing
