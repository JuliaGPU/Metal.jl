...
function MPSVectorDescriptor(length::Integer, dataType::Union{DataType, MPSDataType})
    desc = @objc [
        MPSVectorDescriptor vectorDescriptorWithLength:length::NSUInteger
        dataType:dataType::MPSDataType
    ]::id{MPSVectorDescriptor}
    return MPSVectorDescriptor(desc)
end

function MPSVectorDescriptor(
        length::Integer, vectors, vectorBytes::Integer,
...
function MPSVector(buf, descriptor::MPSVectorDescriptor, offset::Integer = 0)
    ...
    @objc [
        obj::id{MPSVector} initWithBuffer:buf::id{MTLBuffer}
        offset:offset::NSUInteger
        descriptor:descriptor::id{MPSVectorDescriptor}
    ]::id{MPSVector}
    return obj
end
...
function MPSVector(dev::MTLDevice, descriptor::MPSVectorDescriptor)
    ...
    finalizer(release, obj)
    @objc [
        obj::id{MPSVector} initWithDevice:dev::id{MTLDevice}
        descriptor:descriptor::id{MPSVectorDescriptor}
    ]::id{MPSVector}
    return obj
end
...
function MPSTemporaryVector(commandBuffer::MTLCommandBuffer, descriptor::MPSVectorDescriptor)
    obj = @objc [
        MPSTemporaryVector temporaryVectorWithCommandBuffer:commandBuffer::id{MTLCommandBuffer}
        descriptor:descriptor::id{MPSVectorDescriptor}
    ]::id{MPSTemporaryVector}
...
"""
function matvecmul!(
        c::MtlVector, a::MtlMatrix, b::MtlVector, alpha::Number = true, beta::Number = false,
        transpose = false
    )
    ...
    matvec_mul_kernel = MPSMatrixVectorMultiplication(
        device(), !transpose,
        rows_c, cols_a,
        alpha, beta
    )
