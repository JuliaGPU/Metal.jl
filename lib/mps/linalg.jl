...
@autoreleasepool function LinearAlgebra.generic_matmatmul!(
        C::MtlMatrix, tA, tB,
...
@autoreleasepool function LinearAlgebra.generic_matvecmul!(
        C::MtlVector, tA::AbstractChar,
...
@autoreleasepool function LinearAlgebra.lu(
        A::MtlMatrix{T};
        check::Bool = true
    ) where {T <: MtlFloat}
    M, N = size(A)
...
function _check_lu_success(info, allowsingular)
    return if VERSION >= v"1.11.0-DEV.1535"
...
@autoreleasepool function LinearAlgebra.transpose!(
        B::MtlMatrix{T},
        A::MtlMatrix{T}
    ) where {T}
