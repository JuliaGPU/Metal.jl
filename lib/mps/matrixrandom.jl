...
function MPSMatrixRandomNormalDistributionDescriptor(mean, standardDeviation)
    desc = @objc [
        MPSMatrixRandomDistributionDescriptor normalDistributionDescriptorWithMean:mean::Float32
        standardDeviation:standardDeviation::Float32
    ]::id{MPSMatrixRandomDistributionDescriptor}
    obj = MPSMatrixRandomDistributionDescriptor(desc)
    return obj
end
...
function MPSMatrixRandomUniformDistributionDescriptor(minimum, maximum)
    desc = @objc [
        MPSMatrixRandomDistributionDescriptor uniformDistributionDescriptorWithMinimum:minimum::Float32
        maximum:maximum::Float32
    ]::id{MPSMatrixRandomDistributionDescriptor}
    obj = MPSMatrixRandomDistributionDescriptor(desc)
    return obj
...
function encode!(cmdbuf::MTLCommandBuffer, kernel::K, destinationVector::MPSVector) where {K <: MPSMatrixRandom}
    return @objc [
        kernel::id{K} encodeToCommandBuffer:cmdbuf::id{MTLCommandBuffer}
        destinationVector:destinationVector::id{MPSVector}
    ]::Nothing
end
...
for R in [:MPSMatrixRandomMTGP32, :MPSMatrixRandomPhilox]
    @eval begin
        ...
        function $R(device, destinationDataType, seed)
            ...
            finalizer(release, obj)
            @objc [
                obj::id{$R} initWithDevice:device::id{MTLDevice}
        ...
        function $R(device, destinationDataType, seed, distributionDescriptor)
            ...
            @objc [
                ...
                destinationDataType:destinationDataType::MPSDataType
                seed:seed::NSUInteger
                distributionDescriptor:distributionDescriptor::id{MPSMatrixRandomDistributionDescriptor}
            ]::id{$R}
            return obj
        end
    end
...
@inline function _mpsmat_rand!(
        randkern::MPSMatrixRandom, dest::MtlArray{T}, ::Type{T2};
