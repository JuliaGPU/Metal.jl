...
@inline _nrhs(B::MtlVector) = 1
@inline _nrhs(B::MtlMatrix) = size(B, 2)

function _check_square_rhs(A::MtlMatrix, B::MtlVecOrMat)
...
@inline function _uplo_char(uplo::Symbol)
    uplo === :U && return 'U'
    uplo === :L && return 'L'
    throw(ArgumentError("invalid uplo: $uplo"))
end

@inline function _uplo_char(uplo::AbstractChar)
    uplo == 'U' && return 'U'
    uplo == 'L' && return 'L'
    throw(ArgumentError("invalid uplo: $uplo"))
end

@inline _mps_cholesky_lower(uplo) = _uplo_char(uplo) == 'U'
@inline _mps_cholesky_upper(uplo) = _uplo_char(uplo) == 'L'

@autoreleasepool function solve_lu(A::MtlMatrix{T}, B::MtlVecOrMat{T};
                                   check::Bool=true) where {T<:MtlFloat}
    n, nrhs = _check_square_rhs(A, B)
    dev = device()
    queue = global_queue(dev)

    At = MtlMatrix{T, PrivateStorage}(undef, (n, n))
    Bt = _rhs_scratch(T, n, nrhs)
    Xt = _rhs_scratch(T, n, nrhs)
    X = similar(B)
    P = MtlMatrix{UInt32, PrivateStorage}(undef, (1, n))
    status = _status_buffer()
    ...
    cmdbuf = MPSCommandBuffer(queue) do cbuf
        _transpose_copy!(cbuf, dev, mps_a, mps_at, n, n)
        _transpose_copy!(cbuf, dev, mps_b, mps_bt, nrhs, n)
    end
    ...
    commit!(cmdbuf) do cbuf
        _transpose_copy!(cbuf, dev, mps_xt, mps_x, n, nrhs)
    end

    synchronize(cmdbuf)

    info = convert(LinearAlgebra.BlasInt, status[])
    check && _checknonsingular(info)
    return X
end
...
@autoreleasepool function solve_lu(F::LinearAlgebra.LU{T,<:MtlMatrix{T}},
    ...
    cmdbuf = MPSCommandBuffer(queue) do cbuf
        _transpose_copy!(cbuf, dev, mps_lu, mps_lut, n, n)
        _transpose_copy!(cbuf, dev, mps_b, mps_bt, nrhs, n)
    end
    ...
    commit!(cmdbuf) do cbuf
        _transpose_copy!(cbuf, dev, mps_xt, mps_out, n, nrhs)
    end
...
@autoreleasepool function decompose_cholesky!(A::MtlMatrix{T};
    ...
    mps_a = MPSMatrix(A)
    encode!(cmdbuf, MPSMatrixDecompositionCholesky(dev, _mps_cholesky_lower(uplo), n),
            mps_a, mps_a, status)
    commit!(cmdbuf)
    synchronize(cmdbuf)
...
@autoreleasepool function solve_cholesky(C::LinearAlgebra.Cholesky{T,<:MtlMatrix{T}},
    ...
    commitAndContinue!(cmdbuf) do cbuf
        encode!(cbuf, MPSMatrixSolveCholesky(dev, _mps_cholesky_upper(C.uplo), n, nrhs),
                mps_c, mps_bt, mps_xt)
    end
...
@autoreleasepool function solve_cholesky(A::MtlMatrix{T}, B::MtlVecOrMat{T};
    ...
    commitAndContinue!(cmdbuf) do cbuf
        encode!(cbuf, MPSMatrixDecompositionCholesky(dev, _mps_cholesky_lower(uplo), n),
                mps_f, mps_f, status)
    end

    commitAndContinue!(cmdbuf) do cbuf
        encode!(cbuf, MPSMatrixSolveCholesky(dev, _mps_cholesky_upper(uplo), n, nrhs),
                mps_f, mps_bt, mps_xt)
    end
    ...
    info = convert(LinearAlgebra.BlasInt, status[])
    check && _checkpositivedefinite(info)
    return X
end
