...
function MPSMatrixCopyDescriptor(sourceMatrix, destinationMatrix, offsets = MPSMatrixCopyOffsets(Cuint(0), Cuint(0), Cuint(0), Cuint(0)))
    desc = @objc [
        MPSMatrixCopyDescriptor descriptorWithSourceMatrix:sourceMatrix::id{MPSMatrix}
        destinationMatrix:destinationMatrix::id{MPSMatrix}
        offsets:offsets::MPSMatrixCopyOffsets
    ]::id{MPSMatrixCopyDescriptor}
    return MPSMatrixCopyDescriptor(desc)
end
...
function MPSMatrixCopy(dev, copyRows, copyColumns, sourcesAreTransposed, destinationsAreTransposed)
    ...
    @objc [
        obj::id{MPSMatrixCopy} initWithDevice:dev::id{MTLDevice}
        copyRows:copyRows::NSUInteger
        copyColumns:copyColumns::NSUInteger
        sourcesAreTransposed:sourcesAreTransposed::Bool
        destinationsAreTransposed:destinationsAreTransposed::Bool
    ]::id{MPSMatrixCopy}
    return obj
end
...
function encode!(cmdbuf::MTLCommandBuffer, kernel::MPSMatrixCopy, copyDescriptor)
    return @objc [
        kernel::id{MPSMatrixCopy} encodeToCommandBuffer:cmdbuf::id{MTLCommandBuffer}
        copyDescriptor:copyDescriptor::id{MPSMatrixCopyDescriptor}
    ]::Nothing
end
