...
function MPSNDArrayDescriptor(dataType::DataType, dimensionCount, dimensionSizes::Ptr)
    desc = @objc [
        MPSNDArrayDescriptor descriptorWithDataType:dataType::MPSDataType
        dimensionCount:dimensionCount::NSUInteger
        dimensionSizes:dimensionSizes::Ptr{NSUInteger}
    ]::id{MPSNDArrayDescriptor}
...
function transposeDimensionwithDimension(desc::MPSNDArrayDescriptor, dim1, dim2)
    return @objc [
        desc::id{MPSNDArrayDescriptor} transposeDimension:dim1::UInt
        withDimension:dim2::UInt
    ]::Cvoid
end
...
function Base.size(ndarr::MPSNDArray)
    ...
    return Tuple([Int(lengthOfDimension(ndarr, i)) for i in 0:(ndims - 1)])
end
...
function MPSTemporaryNDArray(cmdbuf::MTLCommandBuffer, descriptor::MPSNDArrayDescriptor)
    @objc [
        MPSTemporaryNDArray temporaryNDArrayWithCommandBuffer:cmdbuf::id{MTLCommandBuffer}
        descriptor:descriptor::id{MPSNDArrayDescriptor}
    ]::id{MPSTemporaryNDArray}
    return obj
end
...
function MPSNDArray(device::MTLDevice, desc::MPSNDArrayDescriptor)
    ...
    @objc [
        obj::MPSNDArray initWithDevice:device::id{MTLDevice}
        descriptor:desc::id{MPSNDArrayDescriptor}
    ]::id{MPSNDArray}
    return obj
end
...
@static if Metal.is_macos(v"15")
    function MPSNDArray(buffer::MTLBuffer, offset::UInt, descriptor::MPSNDArrayDescriptor)
        ...
        finalizer(release, obj)
        @objc [
            obj::MPSNDArray initWithBuffer:buffer::id{MTLBuffer}
...
exportDataWithCommandBuffer(ndarr::MPSNDArray, cmdbuf::MTLCommandBuffer, toBuffer, destinationDataType, offset, rowStrides) =
    GC.@preserve rowStrides @objc [
    ndarr::MPSNDArray exportDataWithCommandBuffer:cmdbuf::id{MTLCommandBuffer}
    toBuffer:toBuffer::id{MTLBuffer}
    destinationDataType:destinationDataType::MPSDataType
    offset:offset::NSUInteger
    rowStrides:pointer(rowStrides)::Ptr{NSInteger}
]::Nothing
...
function MPSNDArrayMultiaryKernel(device, sourceCount)
    ...
    finalizer(release, obj)
    @objc [
        obj::id{MPSNDArrayMultiaryKernel} initWithDevice:device::id{MTLDevice}
        sourceCount:sourceCount::NSUInteger
    ]::id{MPSNDArrayMultiaryKernel}
    return obj
end
...
function encode!(cmdbuf::MTLCommandBuffer, kernel::K, sourceArrays, destinationArray) where {K <: MPSNDArrayMultiaryKernel}
    return @objc [
        kernel::id{K} encodeToCommandBuffer:cmdbuf::id{MTLCommandBuffer}
        sourceArrays:sourceArrays::id{NSArray}
        destinationArray:destinationArray::id{MPSNDArray}
    ]::Nothing
end
...
function MPSNDArrayMatrixMultiplication(device, sourceCount)
    ...
    finalizer(release, obj)
    @objc [
        obj::id{MPSNDArrayMatrixMultiplication} initWithDevice:device::id{MTLDevice}
        sourceCount:sourceCount::NSUInteger
    ]::id{MPSNDArrayMatrixMultiplication}
    return obj
end
