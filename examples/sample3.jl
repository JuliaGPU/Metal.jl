...
MTL.assign_argument_buffer!(argbufenc, argbuf1, 1)
MTL.set_field!(argbufenc, 128, 1)
set_buffer!(argbufenc, bufferA.buffer, 0, 2)
content1 = (128, Base.unsafe_load(Base.convert(Ptr{Int}, MTL.content(argbuf1)),2))

argbufenc = MtlArgumentEncoder(fun, 2)
argbuf2 = alloc(Cchar, dev, sizeof(argbufenc), storage=Shared)
MTL.assign_argument_buffer!(argbufenc, argbuf2, 1)
MTL.set_field!(argbufenc, 128, 1)
set_buffer!(argbufenc, bufferB.buffer, 0, 2)
content2 = (128, Base.unsafe_load(Base.convert(Ptr{Int}, MTL.content(argbuf2)),2))

argbufenc = MtlArgumentEncoder(fun, 3)
argbuf3 = alloc(Cchar, dev, sizeof(argbufenc), storage=Shared)
MTL.assign_argument_buffer!(argbufenc, argbuf3, 1)
MTL.set_field!(argbufenc, 128, 1)
set_buffer!(argbufenc, bufferC.buffer, 0, 2)
content3 = (128, Base.unsafe_load(Base.convert(Ptr{Int}, MTL.content(argbuf3)),2))
...
cmd = MTL.commit!(queue) do cmdbuf
    MtlComputeCommandEncoder(cmdbuf) do enc
        MTL.use!(enc, bufferA.buffer, MTL.ReadWriteUsage)
        MTL.use!(enc, bufferB.buffer, MTL.ReadWriteUsage)
        MTL.use!(enc, bufferC.buffer, MTL.ReadWriteUsage)
        #MTL.set_buffers!(enc,
        #                        [argbuf1, argbuf2, argbuf3],
        #                        [0,0,0], 1:3)
        MTL.set_bytes!(enc, pt1, 16, 1)
        MTL.set_bytes!(enc, pt2, 16, 2)
        MTL.set_bytes!(enc, pt3, 16, 3)
